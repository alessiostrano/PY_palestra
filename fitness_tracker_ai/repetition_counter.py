...
class RepetitionCounter:
    ...
    def __init__(self):
        ...
        # Soglie per determinare quando una ripetizione è completata
    ...
    def update(self, exercise_type, evaluation_result):
        ...
        if is_correct:
            self.last_correct_phase = current_phase
    ...
    def _check_squat_completion(self, current_phase, is_correct):
        ...
        if (recent_phases[-3] == 'up' and 
            recent_phases[-2] == 'down' and 
            recent_phases[-1] == 'up'):

            # Verifica che almeno la fase centrale sia stata corretta
            if self.correct_form_required:
                return self.phase_history[-2]['correct'] or is_correct
            return True
    ...
    def _get_timestamp(self):
        """
        Ottiene un timestamp semplice per tracciare le fasi

        Returns:
            int: Timestamp incrementale
        """
    ...
    def get_statistics(self):
        ...
        if not self.phase_history:
            return {'total_reps': self.count, 'correct_form_percentage': 0}
