...
class PostureEvaluator:
    ...
    def evaluate_squat(self, landmarks, pose_detector):
        ...
        try:
            ...
            # Controllo profondità squat e determinazione fase
            if knee_angle > self.thresholds['squat']['knee_max'] - 20:
                phase = 'up'
            elif knee_angle < self.thresholds['squat']['knee_min'] + 20:
                phase = 'down'
                if knee_angle > self.thresholds['squat']['knee_min'] + 40:
            ...
            if abs(right_knee_angle - left_knee_angle) > 25:
                feedback_messages.append("Mantieni le ginocchia allineate!")
                is_correct = False

            # Controllo generale della postura
            if knee_angle < 45:  # Troppo profondo
                feedback_messages.append("Non scendere troppo!")
                is_correct = False
            ...
            print(f"Errore nella valutazione squat: {e}")
            return {'correct': False, 'feedback': f'Errore: {str(e)}', 'phase': 'error'}
    ...
    def evaluate_pushup(self, landmarks, pose_detector):
        ...
        try:
            ...
            # Determinazione fase
            if elbow_angle > self.thresholds['pushup']['elbow_max'] - 30:
                phase = 'up'
            elif elbow_angle < self.thresholds['pushup']['elbow_min'] + 30:
                phase = 'down'
                if elbow_angle > self.thresholds['pushup']['elbow_min'] + 50:
                    feedback_messages.append("Scendi di più!")
                    is_correct = False
            else:
                phase = 'transition'
            ...
            if abs(right_elbow_angle - left_elbow_angle) > 30:
            ...
            print(f"Errore nella valutazione pushup: {e}")
            return {'correct': False, 'feedback': f'Errore: {str(e)}', 'phase': 'error'}
    ...
    def evaluate_bicep_curl(self, landmarks, pose_detector):
        ...
        try:
            ...
            # Determinazione fase
            if elbow_angle > self.thresholds['bicep_curl']['elbow_max'] - 30:
                phase = 'down'
            elif elbow_angle < self.thresholds['bicep_curl']['elbow_min'] + 30:
                phase = 'up'
                if elbow_angle > self.thresholds['bicep_curl']['elbow_min'] + 50:
            ...
            # Controllo simmetria
            if abs(right_elbow_angle - left_elbow_angle) > 35:
            ...
            print(f"Errore nella valutazione bicep curl: {e}")
            return {'correct': False, 'feedback': f'Errore: {str(e)}', 'phase': 'error'}
